...
//敵が出てくるカウント
int TekiAddCnt = 0;
int TekiAddCntMAX = 60;	//60FPSで１回・・・１秒に１回
...
VOID CollUpdateTama(TAMA* tama);			//当たり判定
VOID CollUpdateTeki(CHARACTOR* chara);		//当たり判定の領域を更新
...
BOOL LoadImageDivMem(int* handle, const char* path, int bunkatsuYoko, int bunkatsuTate);	//ゲームの画像の分割読み込み

VOID GameInit(VOID);	//ゲームのデータの初期化

VOID DrawTama(TAMA* tama);		//弾の描画
VOID ShotTama(TAMA* tama, float deg);	//弾の発射
VOID ShotTamaLEFT(TAMA* tama, float deg);	//弾の発射
VOID ShotTamaRIGHT(TAMA* tama, float deg);	//弾の発射
...
BOOL GameLoad(VOID)
{
	...
	//弾のパスをコピー
	strcpyDx(tama_moto.path, ".\\img\\maru_purple.png");
	...
	//背景の画像を読み込み①
	if (LoadImageMem(&back[0], ".\\img\\hosi.jpg") == FALSE) { return FALSE; }
	back[0].x = 0;
	back[0].y = -back[0].height;	//画像の高さ分、位置を上に上げる
	back[0].IsDraw = TRUE;	//描画する

	//背景の画像を読み込み②
	if (LoadImageMem(&back[1], ".\\img\\hosi_rev.jpg") == FALSE) { return FALSE; }
	back[1].x = 0;
	back[1].y = 0;
...
/// <param name="bunkatsuYoko">分割するときの横の数</param>
/// <param name="bunkatsuTate">分割するときの縦の数</param>
/// <returns></returns>
BOOL LoadImageDivMem(int* handle, const char* path, int bunkatsuYoko, int bunkatsuTate)
{
	...
	IsTamaLoad = LoadDivGraph(
		path,		//画像のパス
		TAMA_DIV_MAX,					//分割総数
		bunkatsuYoko, bunkatsuTate,							//横の分割、縦の分割
		TamaWidth / bunkatsuYoko, TamaHeight / bunkatsuTate,	//画像１つ分の幅、高さ
	...
	else
	{
		...
		for (int i = 0; i < TEKI_MAX; i++)
		{
			...
			if (teki[i].img.IsDraw == FALSE)
			{
				...
				teki[i].img.x = GetRand(Bunkatsu  -1) * GAME_WIDTH / 4;
...
/// 弾を飛ばす
/// </summary>
VOID ShotTamaLEFT(TAMA* tama, float deg)
{
	//弾を発射する（描画する）
	tama->IsDraw = TRUE;

	//弾の位置を決める
	tama->StartX = player.img.x;
	tama->StartY = player.img.y + player.img.height / 2 - tama->height / 2;
...
/// 弾を飛ばす
/// </summary>
VOID ShotTamaRIGHT(TAMA* tama, float deg)
{
	//弾を発射する（描画する）
	tama->IsDraw = TRUE;

	//弾の位置を決める
	tama->StartX = player.img.x + player.img.width;
	tama->StartY = player.img.y + player.img.height / 2 - tama->height / 2;
