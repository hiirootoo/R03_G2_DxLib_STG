...
extern BOOL CheckColliPointToMaru(iPOINT pt, MARU m);		//点と円の当たり判定				//点と円の当たり判定
extern BOOL CheckCollMaruToMaru(MARU maru1, MARU maru2);	//円と円の当たり判定

extern RECT GetRect(int left, int top, int right, int bottom);		//RECT型を一時的に流す
extern void DrawRect(RECT r, unsigned int color, bool b);			//RECT型を利用して四角を描画
